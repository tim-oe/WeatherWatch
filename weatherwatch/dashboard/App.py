...
class App:
    ...
    def __init__(self):
        ...
        self._app.layout = html.Div([dcc.Location(id="url"), 
                                     self.sidebar(), 
                                     html.Div(id="page-content", 
                                              style=App.CONTENT_STYLE)])

        self._app.callback(
            Output("page-content", "children"), [Input('url', 'href')]
        )(self.renderContent)
    ...
    def sidebar(self) -> html.Div:
        ...
        links.append(dbc.NavLink("system", id="system", href=SystemPage.PATH, active="exact"))
        links.append(dbc.NavLink("camera", id="camera", href=CameraPage.PATH, active="exact"))
        
        s: SensorConfig
        for s in self._appConfig.sensors:
            links.append(dbc.NavLink(s.name, id=quote(s.name), href=f"/{quote(s.dataClass)}?name={quote(s.name)}", active="exact"))
    ...
    def renderContent(self, href: str):
        f: furl = furl(href)
